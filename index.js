       ...
       case 'lirik':
           lirik(value)
               .then(data => {
                   ...
                   let hasil = `࿕ӃLetra da música *${value}* \n\n\n${lirik}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'fb':
           fb(value)
               .then(data => {
                   ...
                   let hasil = `Escolha a resolução querid@ \n\n\n HD ${resultHD} \n\n\n SD ${resultSD}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui estão os acordes da música *${value}* querida \n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
  ...
  case 'cektanggal':
   ...
   case 'bitly':
           bitly(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui maninh@ ja terminou☣️ :) \n\n${result} `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
